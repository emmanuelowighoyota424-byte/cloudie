...
export interface CryptoProvider { createDepositAddress(input:{customerId:string;asset:string;network:string}):Promise<{address:string;reference?:string}>; getDepositStatus(input:{reference?:string;txHash?:string}):Promise<CryptoDeposit>; verifyTransaction(input:{txHash:string;asset:string;network:string;expectedAmount?:string}):Promise<CryptoDeposit>; normalizeWebhook(payload:unknown):{eventId:string;reference?:string;txHash?:string;status:string}|null }
...
function unitsToDecimal(value:bigint,precision:number){const base=10n**BigInt(precision);const whole=value/base;const fraction=value%base;if(fraction===0n)return whole.toString();return `${whole}.${fraction.toString().padStart(precision,'0').replace(/0+$/,'')}`}
function decimalToUnits(value:string,precision:number){const [whole,fraction='']=value.trim().split('.');if(!/^\d+$/.test(whole)||!/^\d*$/.test(fraction)||fraction.length>precision)throw new Error('Invalid decimal amount');return BigInt(whole)*10n**BigInt(precision)+BigInt((fraction+'0'.repeat(precision)).slice(0,precision)||'0')}
...
export function getCryptoProvider():CryptoProvider{return{
 ...
 async verifyTransaction({txHash,asset,network,expectedAmount}){
  ...
  const tx=await rpcCall('eth_getTransactionByHash',[txHash]) as {to?:string;value?:string;blockNumber?:string}|null;if(!tx)throw new Error('Transaction not found on configured blockchain node')
  ...
  const amount=unitsToDecimal(BigInt(tx.value??'0x0'),decimals)
  if(expectedAmount&&decimalToUnits(amount,decimals)<decimalToUnits(expectedAmount,decimals))return{id:txHash,asset,network,address:depositAddress(),expectedAmount,txHash,confirmations:0,status:'REJECTED'}
