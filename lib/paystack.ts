...
export async function reconcilePaystackPayment(paymentId: string, verified: PaystackTransaction) {
  return prisma.$transaction(async (tx) => {
    ...
    const updated = await tx.payment.update({ where: { id: payment.id }, data: { status, transactionId: String(verified.id), authorization: verified.authorization ? verified.authorization as Prisma.InputJsonValue : undefined, failureReason: status === 'PAID' ? null : verified.gateway_response || verified.status, verifiedAt: new Date(), paidAt: status === 'PAID' ? (verified.paid_at ? new Date(verified.paid_at) : new Date()) : undefined } })
