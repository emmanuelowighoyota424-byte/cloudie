...
export function createCloudiePaymentReference() {
  return `CLD-${Date.now().toString(36).toUpperCase()}-${randomBytes(8).toString('hex').toUpperCase()}`
}
...
export async function reconcilePaystackPayment(paymentId: string, verified: PaystackTransaction) {
  return prisma.$transaction(async (tx) => {
    ...
    const expectedMinor = paystackAmount(payment.amount, payment.order.currency)
    ...
    if (actualCurrency !== payment.order.currency.toUpperCase()) throw new Error('Paystack currency mismatch')

    const success = verified.status.toLowerCase() === 'success'
    const status = success ? 'PAID' : verified.status.toLowerCase() === 'abandoned' ? 'ABANDONED' : 'FAILED'
    const updated = await tx.payment.update({
      where: { id: payment.id },
      data: {
        status,
        transactionId: String(verified.id),
        authorization: verified.authorization ? verified.authorization as Prisma.InputJsonValue : undefined,
        failureReason: success ? null : verified.gateway_response || verified.status,
        verifiedAt: new Date(),
        paidAt: success ? (verified.paid_at ? new Date(verified.paid_at) : new Date()) : undefined,
      },
    })
    if (success) {
      await tx.order.updateMany({ where: { id: payment.orderId, paymentStatus: { not: 'PAID' } }, data: { paymentStatus: 'PAID', status: 'PROCESSING' } })
    }
    await tx.auditLog.create({ data: { userId: payment.order.userId ?? undefined, workspaceId: payment.order.workspaceId, action: `payment.paystack.${status.toLowerCase()}`, entity: 'Payment', entityId: payment.id, metadata: { provider: 'paystack', reference: payment.providerReference, transactionId: verified.id } } })
