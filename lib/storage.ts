...
export class StorageConfigurationError extends Error {
  constructor(message = 'Private document storage is not configured') { super(message); this.name = 'StorageConfigurationError' }
}
...
export function validateUpload(mimeType: string, sizeBytes: number) {
  ...
  if (!Number.isInteger(sizeBytes) || sizeBytes <= 0 || sizeBytes > MAX_UPLOAD_BYTES) throw new Error(`Document exceeds the ${MAX_UPLOAD_BYTES / 1024 / 1024} MB upload limit`)
}
...
export async function storageExists(pathname: string) {
  if (useFilesystem()) {
    try { await fs.access(filePath(pathname)); return true } catch { return false }
  }
  const { bearer, storeId } = resolveAuth()
