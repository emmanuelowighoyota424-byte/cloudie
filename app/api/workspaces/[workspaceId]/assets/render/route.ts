...
export async function POST(request: Request, context: { params: Promise<{ workspaceId: string }> }) {
  ...
  try {
    ...
    charge = await chargeForAction({ userId: user.id, workspaceId, action: 'services.image_render', description: 'Picture Studio image render', reference: `services:image-render:${idempotencyKey}` })
    try {
    ...
    const message = error instanceof Error ? error.message : 'Unable to render asset'
    const status = message.includes('Insufficient points') ? 402 : message.includes('Authentication') ? 401 : 500
