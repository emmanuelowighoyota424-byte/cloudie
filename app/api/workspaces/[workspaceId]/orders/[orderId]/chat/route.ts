...
async function access(workspaceId: string, orderId: string) {
  ...
  const order = await prisma.order.findFirst({ where: { id: orderId, workspaceId }, select: { id: true, userId: true } })
  if (!order) throw new Error('Order not found')
  if (order.userId !== user.id) {
    const member = await prisma.workspaceMember.findUnique({ where: { workspaceId_userId: { workspaceId, userId: user.id } }, select: { role: true, status: true } })
    if (!member || member.status !== 'ACTIVE' || !['SUPER_ADMIN','WORKSPACE_ADMIN','MANAGER','STAFF','VENDOR'].includes(member.role)) throw new Error('Order access denied')
  }
  return { user, order }
}
...
async function ensureConversation(workspaceId: string, orderId: string, userId: string, ownerId: string | null) {
  const conversation = await prisma.$transaction(async (tx) => {
    const existing = await tx.$queryRaw<Array<{ id: string }>>`SELECT "id" FROM "OrderChatConversation" WHERE "orderId"=${orderId} LIMIT 1`
    const id = existing[0]?.id ?? crypto.randomUUID()
    if (!existing[0]) await tx.$executeRaw`INSERT INTO "OrderChatConversation"("id","orderId","workspaceId") VALUES (${id},${orderId},${workspaceId})`
    await tx.$executeRaw`INSERT INTO "OrderChatParticipant"("id","conversationId","userId") VALUES (${crypto.randomUUID()},${id},${userId}) ON CONFLICT ("conversationId","userId") DO NOTHING`
    if (ownerId) await tx.$executeRaw`INSERT INTO "OrderChatParticipant"("id","conversationId","userId") VALUES (${crypto.randomUUID()},${id},${ownerId}) ON CONFLICT ("conversationId","userId") DO NOTHING`
    return id
  })
  return conversation
}
...
export async function GET(_request: Request, context: { params: Promise<{ workspaceId: string; orderId: string }> }) {
  try {
    ...
    const messages = await prisma.$queryRaw<Array<{ id:string; conversationId:string; senderId:string; body:string; createdAt:Date }>`SELECT "id","conversationId","senderId","body","createdAt" FROM "OrderChatMessage" WHERE "conversationId"=${conversationId} ORDER BY "createdAt" ASC LIMIT 500`
    const participant = await prisma.$queryRaw<Array<{ lastReadAt:Date|null }>>`SELECT "lastReadAt" FROM "OrderChatParticipant" WHERE "conversationId"=${conversationId} AND "userId"=${user.id} LIMIT 1`
    const unread = await prisma.$queryRaw<Array<{ count: bigint }>>`SELECT COUNT(*)::bigint AS count FROM "OrderChatMessage" WHERE "conversationId"=${conversationId} AND "senderId"<>${user.id} AND (${participant[0]?.lastReadAt ? prisma.$queryRaw`"createdAt">${participant[0].lastReadAt}` : prisma.$queryRaw`TRUE`})`
...
export async function POST(request: Request, context: { params: Promise<{ workspaceId: string; orderId: string }> }) {
  try {
    ...
    const conversationId = await ensureConversation(workspaceId, orderId, user.id, order.userId)
    const rows = await prisma.$queryRaw<Array<{ id:string; conversationId:string; senderId:string; body:string; createdAt:Date }>>`
...
export async function PATCH(_request: Request, context: { params: Promise<{ workspaceId: string; orderId: string }> }) {
  try {
    ...
    const { user, order } = await access(workspaceId, orderId)
    const conversationId = await ensureConversation(workspaceId, orderId, user.id, order.userId)
    await prisma.$executeRaw`UPDATE "OrderChatParticipant" SET "lastReadAt"=CURRENT_TIMESTAMP WHERE "conversationId"=${conversationId} AND "userId"=${user.id}`
