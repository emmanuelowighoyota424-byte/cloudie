...
export async function GET(_: Request, context: { params: Promise<{ workspaceId: string }> }) {
  try {
    ...
    await requireWorkspaceRole(workspaceId, canManage as never)
    const shipments = await prisma.shipment.findMany({
      where: { workspaceId },
      include: { customer: true, driver: true, warehouse: true, events: { orderBy: { createdAt: 'desc' }, take: 1 } },
...
export async function POST(request: Request, context: { params: Promise<{ workspaceId: string }> }) {
  try {
    ...
    const [customer, driver, warehouse] = await Promise.all([
      ...
      driverId ? prisma.driver.findFirst({ where: { id: driverId, workspaceId }, select: { id: true } }) : null,
      warehouseId ? prisma.warehouse.findFirst({ where: { id: warehouseId, workspaceId }, select: { id: true } }) : null,
    ])
