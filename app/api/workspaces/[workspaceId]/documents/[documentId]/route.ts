...
async function authorizedDocument(workspaceId: string, documentId: string) {
  ...
  const document = await prisma.document.findFirst({ where: { id: documentId, workspaceId }, include: { shipment: { select: { customer: { select: { email: true } }, driver: { select: { userId: true } }, warehouseId: true } }, customer: { select: { email: true } } } })
  ...
  const isWarehouseStaff = membership.role === 'WAREHOUSE_STAFF' && document.shipment?.warehouseId ? Boolean(await prisma.warehouseStaffAssignment.findFirst({ where: { workspaceId, userId: user.id, warehouseId: document.shipment.warehouseId } })) : false
...
export async function GET(_: Request, context: { params: Promise<{ workspaceId: string; documentId: string }> }) {
  try {
    ...
    return new Response(response.body, { status: 200, headers: { 'content-type': document.mimeType, 'content-disposition': `inline; filename="${document.name.replace(/"/g, '')}"`, 'cache-control': 'private, no-store', 'x-content-type-options': 'nosniff' } })
  } catch (error) {
