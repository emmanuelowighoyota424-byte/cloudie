...
const WRITE_ROLES = [WorkspaceRole.SUPER_ADMIN, WorkspaceRole.WORKSPACE_ADMIN, WorkspaceRole.MANAGER, WorkspaceRole.STAFF]
...
export async function POST(request: Request) {
  ...
  if (!workspaceId) return NextResponse.json({ error: 'workspaceId is required' }, { status: 400 })
  try {
    const { user } = await requireWorkspaceRole(workspaceId, WRITE_ROLES)
    ...
    const status = message.includes('Authentication') ? 401 : message.includes('access') || message.includes('permissions') ? 403 : message.includes('belong') ? 400 : 500
