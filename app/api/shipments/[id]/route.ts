...
function statusFor(error: unknown) {
  ...
  if (message === 'Authentication required' || message === 'Account suspended') return 401
  if (message.includes('access denied') || message.includes('permissions')) return 403
  if (message === 'Shipment changed concurrently') return 409
  return 500
}
...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ ok: false, error: { code: 'SHIPMENT_READ_FAILED', message: error instanceof Error ? error.message : 'Unable to load shipment' } }, { status: statusFor(error) })
  }
}
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await prisma.$transaction(async tx => {
      // Optimistic concurrency guard: only the status we authorized may be replaced.
      // This prevents two concurrent requests from both validating the same stale status.
      const changed = await tx.shipment.updateMany({
        where: { id, status: shipment.status },
        data: { status: status as ShipmentStatus, deliveredAt: status === 'DELIVERED' ? new Date() : undefined },
      })
      if (changed.count !== 1) throw new Error('Shipment changed concurrently')
    ...
    return NextResponse.json({ ok: false, error: { code: 'SHIPMENT_UPDATE_FAILED', message: error instanceof Error ? error.message : 'Unable to update shipment' } }, { status: statusFor(error) })
  }
}
