...
export default async function AdminPage() {
  ...
  const [users, activeUsers, suspendedUsers, vendors, shipments, deliveredShipments, documents, workspaces, orders, disputes, pendingKyc, failedJobs, points] = await Promise.all([
    ...
    prisma.kYCVerification.count({ where: { status: { in: ['PENDING','UNDER_REVIEW'] } } }),
    prisma.cloudieJob.count({ where: { status: 'FAILED' } }), prisma.pointLedger.aggregate({ _sum: { amount: true } }),
  ])
