    ...
    <div className="lg:pl-72">
      <header className="sticky top-0 z-30 border-b bg-background/95 px-4 py-3 backdrop-blur sm:px-6">
        <div className="flex items-center gap-3">
          ...
          <nav className="hidden min-w-0 flex-1 gap-2 overflow-x-auto pb-1" aria-label="Admin navigation">{sections.map(([label,href])=><Link key={href} href={href} className="whitespace-nowrap rounded-full border bg-background px-3 py-1.5 text-xs font-medium hover:bg-muted">{label}</Link>)}</nav>
