...
export default async function TrackingPage({ params }: { params: Promise<{ trackingNumber: string }> }) {
  ...
  if (!/^CLD-[A-F0-9]{10}$/.test(normalized)) notFound()
  const shipment = await prisma.shipment.findUnique({
    where: { trackingId: normalized },
    select: { trackingId: true, status: true, origin: true, destination: true, createdAt: true, deliveredAt: true, events: { select: { status: true, location: true, createdAt: true }, orderBy: { createdAt: 'desc' } } },
  })
  ...
  return (
    <main className="mx-auto min-h-screen max-w-3xl px-4 py-12 sm:px-6">
      <div className="rounded-2xl border bg-background p-6 shadow-sm sm:p-8">
        ...
        <section className="mt-8"><h2 className="text-lg font-semibold">Shipment history</h2><ol className="mt-4 space-y-4">{shipment.events.map((event) => <li key={`${event.createdAt.toISOString()}-${event.status}`} className="rounded-xl border p-4"><div className="flex flex-wrap items-center justify-between gap-2"><span className="font-medium">{event.status.replaceAll('_', ' ')}</span><time className="text-sm text-muted-foreground">{event.createdAt.toLocaleString()}</time></div>{event.location && <p className="mt-1 text-sm text-muted-foreground">{event.location}</p>}</li>)}</ol></section>
      </div>
    </main>
  )
}
