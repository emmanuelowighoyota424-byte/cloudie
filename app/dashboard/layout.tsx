...
import { Bell, Boxes, CreditCard, FileText, Home, LifeBuoy, Package, ShoppingBag, Ticket, UserRound, WalletCards } from 'lucide-react'
...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <div className="sticky top-0 z-50 border-b bg-background/90 backdrop-blur-xl">
      <DashboardHeader points={points} unread={unread} userName={user.name} />
    </div>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 border-t bg-background/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-xl lg:hidden"><div className="mx-auto grid max-w-lg grid-cols-5 gap-1">
      ...
      ].map(([label, href, Icon]) => <Link key={href} href={href} className="flex min-h-12 flex-col items-center justify-center gap-1 rounded-xl text-[10px] font-semibold text-muted-foreground hover:bg-muted hover:text-foreground"><Icon className="size-4" />{label}</Link>)}
    </div></nav>
  </div>
}
