...
ALTER TABLE "RealtimeTicket" ADD CONSTRAINT "RealtimeTicket_sessionId_fkey" FOREIGN KEY ("sessionId") REFERENCES "Session"("id") ON DELETE CASCADE;
ALTER TABLE "RealtimeTicket" ADD CONSTRAINT "RealtimeTicket_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE;
ALTER TABLE "RealtimeTicket" ADD CONSTRAINT "RealtimeTicket_workspaceId_fkey" FOREIGN KEY ("workspaceId") REFERENCES "Workspace"("id") ON DELETE CASCADE;
ALTER TABLE "OrderChatConversation" ADD CONSTRAINT "OrderChatConversation_orderId_fkey" FOREIGN KEY ("orderId") REFERENCES "Order"("id") ON DELETE CASCADE;
ALTER TABLE "OrderChatConversation" ADD CONSTRAINT "OrderChatConversation_workspaceId_fkey" FOREIGN KEY ("workspaceId") REFERENCES "Workspace"("id") ON DELETE CASCADE;
ALTER TABLE "OrderChatParticipant" ADD CONSTRAINT "OrderChatParticipant_conversationId_fkey" FOREIGN KEY ("conversationId") REFERENCES "OrderChatConversation"("id") ON DELETE CASCADE;
ALTER TABLE "OrderChatParticipant" ADD CONSTRAINT "OrderChatParticipant_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE CASCADE;
ALTER TABLE "OrderChatMessage" ADD CONSTRAINT "OrderChatMessage_conversationId_fkey" FOREIGN KEY ("conversationId") REFERENCES "OrderChatConversation"("id") ON DELETE CASCADE;
ALTER TABLE "OrderChatMessage" ADD CONSTRAINT "OrderChatMessage_senderId_fkey" FOREIGN KEY ("senderId") REFERENCES "User"("id") ON DELETE CASCADE;

CREATE OR REPLACE FUNCTION cloudie_emit_realtime_event() RETURNS trigger AS $$
DECLARE row_json jsonb; user_id_value text; workspace_id_value text; event_name text := TG_ARGV[0]; entity_name text := TG_ARGV[1];
BEGIN
  row_json := to_jsonb(NEW); user_id_value := NULLIF(row_json->>'userId',''); workspace_id_value := NULLIF(row_json->>'workspaceId','');
  IF event_name = 'AUTO' THEN event_name := CASE WHEN TG_OP='INSERT' THEN TG_ARGV[2] ELSE TG_ARGV[3] END; END IF;
  IF event_name = 'AUTO_STATUS' THEN
    IF COALESCE(to_jsonb(OLD)->>'status','') = COALESCE(row_json->>'status','') THEN RETURN NEW; END IF;
    event_name := TG_ARGV[2];
  END IF;
  INSERT INTO "RealtimeEvent"("eventType","entityType","entityId","userId","workspaceId","payload") VALUES (event_name,entity_name,COALESCE(row_json->>'id',row_json->>'shipmentId'),user_id_value,workspace_id_value,jsonb_build_object('operation',TG_OP));
  PERFORM pg_notify('cloudie_realtime','event'); RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION cloudie_emit_shipment_event() RETURNS trigger AS $$
DECLARE shipment_row jsonb;
BEGIN
  SELECT to_jsonb(s) INTO shipment_row FROM "Shipment" s WHERE s.id=NEW."shipmentId";
  INSERT INTO "RealtimeEvent"("eventType","entityType","entityId","userId","workspaceId","payload") VALUES ('SHIPMENT_EVENT_CREATED','ShipmentEvent',NEW.id,shipment_row->>'customerId',shipment_row->>'workspaceId',jsonb_build_object('operation','INSERT'));
  PERFORM pg_notify('cloudie_realtime','event'); RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION cloudie_emit_order_message_event() RETURNS trigger AS $$
DECLARE conversation_row jsonb;
BEGIN
  SELECT to_jsonb(c) INTO conversation_row FROM "OrderChatConversation" c WHERE c.id=NEW."conversationId";
  INSERT INTO "RealtimeEvent"("eventType","entityType","entityId","userId","workspaceId","payload") VALUES ('ORDER_MESSAGE_CREATED','OrderChatMessage',NEW.id,NULL,conversation_row->>'workspaceId',jsonb_build_object('operation','INSERT'));
  PERFORM pg_notify('cloudie_realtime','event'); RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
CREATE TRIGGER realtime_order_created AFTER INSERT ON "Order" FOR EACH ROW EXECUTE FUNCTION cloudie_emit_realtime_event('AUTO','Order','ORDER_CREATED','ORDER_UPDATED');
CREATE TRIGGER realtime_order_updated AFTER UPDATE ON "Order" FOR EACH ROW EXECUTE FUNCTION cloudie_emit_realtime_event('AUTO','Order','ORDER_CREATED','ORDER_UPDATED');
...
CREATE TRIGGER realtime_jobs AFTER INSERT OR UPDATE ON "CloudieJob" FOR EACH ROW EXECUTE FUNCTION cloudie_emit_realtime_event('JOB_STATUS_CHANGED','CloudieJob');
