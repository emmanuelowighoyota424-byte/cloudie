...
function parseFrame(buffer:Buffer){if(buffer.length<2)return null;let length=buffer[1]&0x7f;let offset=2;if(length===126){if(buffer.length<4)return null;length=buffer.readUInt16BE(2);offset=4}if(length===127)throw new Error('test frame too large');if(buffer.length<offset+length)return null;return{value:JSON.parse(buffer.subarray(offset,offset+length).toString()) as Record<string,unknown>,bytes:offset+length}}
...
test('persistent realtime server authenticates, scopes events and replays outbox events',async()=>{
 ...
 const ticketA=await makeTicket(userA.id,sessionA.id,workspace.id);const ticketB=await makeTicket(userB.id,sessionB.id,workspace.id)
 child=spawn('node',['realtime/server.mjs'],{env:{...process.env,PORT:String(port),REALTIME_ALLOWED_ORIGINS:origin},stdio:'ignore'})
 try{
  await waitForHealth();const a=await connect(ticketA);const b=await connect(ticketB);await a.wait(v=>v.type==='ready');await b.wait(v=>v.type==='ready')
  a.socket.write(maskedFrame(JSON.stringify({type:'subscribe',workspaceId:workspace.id,since:0})));b.socket.write(maskedFrame(JSON.stringify({type:'subscribe',workspaceId:workspace.id,since:0})));await a.wait(v=>v.type==='subscribed');await b.wait(v=>v.type==='subscribed')
  ...
  const eventA=await a.wait(v=>v.type==='event'&&v.entityId===notificationA.id);assert.equal(eventA.eventType,'NOTIFICATION_CREATED')
  const bLeak=await Promise.race([b.wait(v=>v.type==='event'&&v.entityId===notificationA.id).then(()=>true),new Promise<boolean>(r=>setTimeout(()=>r(false),500))]);assert.equal(bLeak,false)
  const cursor=Number(eventA.id);a.socket.end();b.socket.end()
  const sessionA2=await prisma.session.create({data:{id:crypto.randomUUID(),token:crypto.randomUUID(),userId:userA.id,expiresAt:new Date(Date.now()+3600000)}});const ticketA2=await makeTicket(userA.id,sessionA2.id,workspace.id);const replay=await connect(ticketA2);await replay.wait(v=>v.type==='ready');replay.socket.write(maskedFrame(JSON.stringify({type:'subscribe',workspaceId:workspace.id,since:Math.max(0,cursor-1)})));const replayed=await replay.wait(v=>v.type==='event'&&v.entityId===notificationA.id);assert.equal(replayed.eventType,'NOTIFICATION_CREATED');assert.ok(Number(replayed.id)>=cursor);replay.socket.end();await prisma.session.delete({where:{id:sessionA2.id}})
  await prisma.notification.deleteMany({where:{id:{in:[notificationA.id,notificationB.id]}}});await prisma.session.deleteMany({where:{id:{in:[sessionA.id,sessionB.id]}}});await prisma.workspaceMember.deleteMany({where:{workspaceId:workspace.id}});await prisma.workspace.delete({where:{id:workspace.id}});await prisma.user.deleteMany({where:{id:{in:[userA.id,userB.id]}}})
